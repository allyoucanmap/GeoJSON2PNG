...
module.exports = {
    set: (_parent = document.body, _width, _height) => {
        ...
        documentation.onmouseover = () => {
            documentation.style.opacity =  1.0;
        };
        documentation.onmouseout = () => {
            documentation.style.opacity = 0.5;
        };
        ...
        overlay.innerHTML =
        '<div style="height:' + height * 3 / 5 + 'px;width:' + width * 2 / 4 + 'px;">' +
          h2('DOCUMENTATION') +
          h3("What's GeoJSON2PNG") +
          p('GeoJSON2PNG is a GeoJSON to PNG converter playground.') +
          h3('Interactions') +
          p('Drag and drop a GeoJSON file* (supported only Polygon and MultiPolygon geometries)') +
          p('Click on START to initialize a new game session') +
          p('Move your feature (grey feature named player)') +
          p('- press [W] or [Arrow Up] to move UP<br/>- press [A] or [Arrow Left] to move LEFT<br/>- press [D] or [Arrow Right] to move RIGHT') +
          p('Jump on top of a feature to catch it') +
          p('Retrieve your PNG file at the end of game session from PNG box') +
          p('* If you have not a GeoJSON file you can try the game by clicking on PRACTICE') +
          p(' _____ ') +
          p('Click to close the documentation') +
        '</div>';
